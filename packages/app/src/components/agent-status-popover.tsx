            ...
            <View key={s} style={styles.group}>
              ...
              {grouped[s].map((a) => (
                <Pressable key={a.id} style={styles.row}>
                  ...
                  {a.workspace ? <Text style={styles.rowMeta} numberOfLines={1}>· {a.workspace}</Text> : null}
                </Pressable>
              ))}
            </View>
          ))
...
const styles = StyleSheet.create({
  ...
  groupHeader: { color: C.fgSubtle, fontSize: FS.xs, fontWeight: FW.semibold, marginBottom: 4 },
  row: {
